...
class NormalZombie(Animatable):
    ...
    def spawn(self, *, start_ms: float = 0):
        self.set_animation_set(NormalZombie.SPAWN_ANIMATION_SET, start_ms=start_ms or pygame.time.get_ticks())

    def despawn(self, *, start_ms: float = 0):
        self.set_animation_set(NormalZombie.DESPAWN_ANIMATION_SET, start_ms=start_ms)
