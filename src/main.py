...
from objects.Level.Level import (
    LevelHandle,
    WIN,
    LOSE,
    TIME_UP,
...
while running:
    ...
    if (
        level_object.get_game_state() == LOSE
        or level_object.get_game_state() == TIME_UP
    ) and level_object.get_current_scene() != MENU_SCENE:
        screen.blit(SPRITE_MAP[GRASS_IDX], (0, 0))

        point_object.draw_ending_scene(level_object, time_object)
        continue
    elif (
        level_object.get_game_state() == WIN
        and level_object.get_current_scene() != MENU_SCENE
    ):
        screen.blit(SPRITE_MAP[GRASS_IDX], (0, 0))
    ...
    # State update stage #
    if quit_screen:
        screen.blit(BACKGROUND_MAIN_IMAGE, (0, 0))
        display_quit_screen()
        pygame.display.flip()
        continue
    ...
    if last_mouse_state[0] == 0 and current_mouse_state[0] == 1:
        if start:
            ...
            for spot in spawning_spots:
                ...
                if active_zombie is not None and active_zombie.get_rect(
                    SPRITE_MAP
    ...
    if level_object.get_current_scene() == "Menu":
        ...
        # Display the point
        point_object.display_hit_miss(
            point_object.get_hits(), point_object.get_misses(), 10
        )
        # Render timer
